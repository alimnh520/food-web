...
const Header = () => {
    ...
    return (
        <div className='w-full flex flex-col bg-red-50 relative'>
            <TopHead />
            <div className="h-10 w-28 bg-gray-600 absolute right-5 top-20 cursor-pointer text-lg font-bold text-white hidden sm:flex items-center justify-between px-2" onClick={() => setNav(!showNav)}>
                MENU
                <GiHamburgerMenu className='text-2xl'/>
            </div>
            <div className={`w-full px-20 h-16 flex items-center justify-between transition-all duration-300 ${showNav ? "sm:h-80" : "sm:h-14"} sm:overflow-hidden sm:justify-start sm:flex-col sm:items-start sm:px-5`}>
                ...
                <ul className={`flex items-center justify-center space-x-5 font-semibold text-sm sm:flex-col sm:items-start sm:space-y-5 sm:space-x-0 sm:w-full sm:pt-5`}>
                    ...
                    <li><Link href="/components/shop-page" className={`${linkCls} transition-all duration-500`}>Shop</Link></li>
